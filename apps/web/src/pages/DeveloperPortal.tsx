...
export default function DeveloperPortal() {
  ...
  const services = [
    { id: 'seo-agent', name: 'SEO Strategy Node', category: 'Marketing', price: 6, usage: 1450, revenue: 8700, status: 'Active' },
    { id: 'copywriter', name: 'Neural Copy Engine', category: 'Writing', price: 5, usage: 2100, revenue: 10500, status: 'Active' },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-16 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         {[
           { label: 'Total Revenue', value: '19,200', unit: 'AXC', icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-400/10', desc: 'Gross revenue across all nodes.' },
           { label: 'Network Invocations', value: '3,550', unit: 'Calls', icon: Zap, color: 'text-primary', bg: 'bg-primary/10', desc: 'Total successful autonomous hires.' },
           { label: 'Avg Node Yield', value: '5.4', unit: 'AXC/Call', icon: DollarSign, color: 'text-amber-400', bg: 'bg-amber-400/10', desc: 'Net earnings per capability call.' },
         ].map((stat, i) => (
           <motion.div
             key={i}
              ...
              <div className="flex items-center gap-8 mb-10 relative z-10">
                 ...
                 <div className="space-y-1">
                    ...
                    <div className="flex items-baseline gap-2">
                       <p className="text-4xl font-black tracking-tighter text-white group-hover:text-primary transition-colors">{stat.value}</p>
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[800px]">
                 ...
                 <tbody className="divide-y divide-white/[0.05]">
                    {services.map(s => (
                       <tr key={s.id} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                          <td className="px-10 py-10">
                             <div className="flex items-center gap-6">
                                ...
                                <div className="space-y-1">
                                   ...
                                   <p className="text-[10px] font-bold text-charcoal-500 uppercase tracking-[0.2em]">{s.category}</p>
                                </div>
                             </div>
                          </td>
                          ...
                          <td className="px-10 py-10 text-center">
                             <span className="text-xl font-black text-charcoal-300 tracking-tight">{s.usage.toLocaleString()}</span>
                          </td>
                          <td className="px-10 py-10 text-center">
                             <span className="text-2xl font-black text-emerald-400">+{s.revenue.toLocaleString()} <span className="text-[10px] opacity-40 italic">AXC</span></span>
                          </td>
                          <td className="px-10 py-10">
                             <Badge variant="success" className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 px-4 py-1.5 rounded-xl font-black uppercase tracking-widest text-[9px]">ENFORCED: {s.status}</Badge>
                          </td>
